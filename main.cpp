...
int main(int argc, char** argv) {
    ...
    Sor * sorProcessor = new Sor(from, len, filename);
    //if -print_col_type input was valid
    if (printColType != UINT_MAX) {
        sorProcessor->printColumnType(printColType);
    }
    
    ////if -print_col_idx input was valid
    if (printCell) {
        sorProcessor->printValue(printColIndexCol, printColIndexOffset);
    }

    //if -is_missing_idx input was valid
    if (printIsMissing) {
        sorProcessor->printIsMissing(isMissingCol, isMissingOffset);
    }
